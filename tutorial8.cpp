...
static void start_feed([[maybe_unused]] GstElement* source,
                       [[maybe_unused]] guint size, CustomData* data) {
  if (data->sourceid == 0) {
...
static void stop_feed([[maybe_unused]] GstElement* source, CustomData* data) {
  if (data->sourceid != 0) {
...
static void error_cb([[maybe_unused]] GstBus* bus, GstMessage* msg,
                     CustomData* data) {
  GError* err;
  gchar* debug_info;
...
int main(int argc, char* argv[]) {
  ...
  /* Configure wavescope */
  g_object_set(data.visual, "shader", 0, "style", 0, nullptr, nullptr);
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), data.app_source, data.tee,
                   ...
                   data.visual, data.video_convert, data.video_sink,
                   data.app_queue, data.app_sink, nullptr);

  if (!gst_element_link_many(data.app_source, data.tee, nullptr) ||
      !gst_element_link_many(data.audio_queue, data.audio_convert1,
                             data.audio_resample, data.audio_sink, nullptr) ||
      !gst_element_link_many(data.video_queue, data.audio_convert2, data.visual,
                             data.video_convert, data.video_sink, nullptr) ||
      !gst_element_link_many(data.app_queue, data.app_sink, nullptr)) {
