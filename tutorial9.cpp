...
static void print_tag_foreach(const GstTagList* tags, const gchar* tag,
  ...
  if (G_VALUE_HOLDS_STRING(&val)) str = g_value_dup_string(&val);
  else
...
int main(int argc, char** argv) {
  ...
  char default_uri[] = "https://gstreamer.freedesktop.org/data/media/"
                       "sintel_trailer-480p.webm";

  gchar* uri = default_uri;
  ...
  g_signal_connect(data.discoverer, "discovered", G_CALLBACK(on_discovered_cb),
                   &data);
